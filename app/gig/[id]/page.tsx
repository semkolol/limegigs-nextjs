...
async function GetGig({ id }: { id: string }) {
...
export default async function GigPage({ params }: { params: { id: string } }) {
    ...
    const gig = await GetGig(params);

    return (
        <div className='py-10'>
            {gig &&
                <div className="flex flex-col mx-auto w-full max-w-6xl p-2 space-y-2 items-center">
                    <div className='py-10 flex flex-col items-center space-x-2 justify-evenly'>
                        <div className='flex flex-col items-center'>
                            <Image src={`https://rckgfebgmiatwsmkwheb.supabase.co/storage/v1/object/public/company_logos/${gig.company_logo}`} alt={gig.company_name} width={100} height={100} />
                            <div className='flex flex-col'>
                                <span className='font-bold text-xl'>{gig.gig_title}</span>
                                <span className='text-neutral-300 text-lg'>{gig.company_name}</span>
                                <div className='flex flex-col md:flex-row space-x-2 items-start md:items-center'>
                                    ...
                                    <div className='space-y-2 '>
                                        <div className='flex flex-row space-x-2'>
                                            ...
                                            <span className='px-2 rounded-lg bg-violet-200 dark:bg-violet-900 capitalize'>Company Size <b>{gig.company_size}</b></span>
                                        </div>
                                        <div className='flex'>
                        ...
                        <div className='pt-10 space-x-2 flex items-center'>
                            {gig.link &&
